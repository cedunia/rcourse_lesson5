...
data_stats <- data_clean %>%
  ...
  summarise(perc_incumbent_mean = mean(perc_votes_incumbent, 
                                       na.rm = T)) %>%
  ungroup()
...
# ezANOVA
incumbent.ezanova <- ezANOVA(data.frame(data_stats), dv = perc_incumbent_mean,
                             wid = state, within = incumbent_party,
                             between = civil_war,
                             type = 3)
incumbent.ezanova
...
incumbent_union.ttest <- t.test(data_union_stats$democrat, 
                                data_union_stats$republican, paired = T)
incumbent_union.ttest

incumbent_confederacy.ttest <- t.test(data_confederacy_stats$democrat,
                                      data_confederacy_stats$republican, paired = T)
incumbent_confederacy.ttest

# Effect of civil war, separated by incumbent party
